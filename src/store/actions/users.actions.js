  ...
  return (dispatch) => {
    axios
      ...
      .then((response) => {
        ...
        dispatch({ type: "USER_SIGNIN_SUCCESS", userDets, token: response.data.token });
      })
      .catch((err) => {
...
export const validateToken = (url) => {
  return (dispatch) => {
    ...
    if (token) {
      axios
        .post("/simple-jwt-authentication/v1/token/validate", {}, { headers: { Authorization: "Bearer " + token } })
        .then((res) => {
          if (res.data.data.status === 200) {
            dispatch({ type: actionTypes.VALIDATE_TOKEN_SUCCESS, token, email, redirectTo: url });
          }
        })
        .catch((err) => {
